...
case ASTNodeType.ForStatement: {
  const forNode = node as ASTForStatement;

  evaluate(forNode.initializer, environment);

  let result: InterpreterValue = { type: InterpreterValueType.Null, value: null } as InterpreterNull;

  while (true) {
    const conditionVal = evaluate(forNode.condition, environment);

    if (conditionVal.type === InterpreterValueType.Boolean) {
      if (!(conditionVal as InterpreterBoolean).value) break;
    } else if (conditionVal.type === InterpreterValueType.Number) {
      if ((conditionVal as InterpreterNumber).value === 0) break;
    } else {
      throw new Error("La condition du 'for' doit être un booléen ou un nombre");
    }

    const loopEnv = new Environment(environment);
    for (const statement of forNode.body) {
      result = evaluate(statement, loopEnv);
    }

    evaluate(forNode.increment, environment);
  }
